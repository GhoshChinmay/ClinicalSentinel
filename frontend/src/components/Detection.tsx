                        ...
                        <table className="w-full text-sm text-left table-fixed">
                            ...
                            <tbody className="divide-y divide-neutral-800">
                                {displayData.map((row, idx) => {
                                    ...
                                    return (
                                        <motion.tr initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: idx * 0.02 }} key={idx} className="hover:bg-neutral-900/40">
                                            <td className="px-6 py-5 w-[45%] align-top">
                                                <div className="grid grid-cols-2 lg:grid-cols-3 gap-2 pr-2">
                                                    {Object.entries(rowData).map(([key, val]) => (
                                                        <div key={key} className="flex flex-col bg-black/40 p-2.5 rounded-lg border border-neutral-800/60 min-w-0">
                                                            <span className="text-[9px] text-neutral-500 uppercase font-bold tracking-wider mb-1 truncate" title={key}>{key}</span>
                                                            <span className="text-xs font-mono text-neutral-200 truncate" title={String(val)}>
                                                ...
                                                <div className="flex flex-col space-y-3">
                                                    ...
                                                    <div className="flex items-start text-neutral-300 bg-purple-900/10 p-4 rounded-xl border border-purple-500/20 shadow-inner">
                                                        ...
                                                        <span className="text-sm leading-relaxed font-medium">
                                                            {row.AI_Reason || "Flagged as an anomaly by the multi-dimensional detection model."}
                                                        </span>
                                                    </div>
                                                                ...
                                                                {shapData.map((shap, sIdx) => {
                                                                    ...
                                                                    return (
                                                                        <div key={sIdx} className="flex items-center justify-between group">
                                                                            <span className="text-xs text-neutral-400 font-mono truncate w-1/3 pr-2" title={shap.feature}>
                                                                                {shap.feature}
                                                                            </span>
                                                                            ...
                                                                            <span className={`text-[10px] font-mono w-12 text-right ${isPositive ? 'text-red-400' : 'text-emerald-400'}`}>
                                                                                {isPositive ? '+' : ''}{shap.impact.toFixed(2)}
                                                                            </span>
                                                                        </div>
                                                                    );
                                                                })}
                                                    ...
                                                    <div className="flex items-center space-x-2 pt-1">
                                                        <span className="text-[10px] uppercase font-bold text-neutral-600 mr-2">Was this correct?</span>
                                                        <button
                                                            onClick={() => handleFeedback(row, originalIdx, true)}
                                                            disabled={feedbackMap[originalIdx] !== undefined}
                                                            className={`flex items-center px-3 py-1.5 space-x-1.5 rounded-lg text-xs font-bold border transition-colors ${feedbackMap[originalIdx] === 'correct' ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/50' : feedbackMap[originalIdx] === 'incorrect' ? 'opacity-30 cursor-not-allowed bg-black text-neutral-600 border-neutral-800' : 'bg-[#0A0A0A] text-neutral-400 border-neutral-800 hover:text-emerald-400 hover:border-emerald-500/50'}`}
                                                        >
                                                        ...
                                                        <button
                                                            ...
                                                            disabled={feedbackMap[originalIdx] !== undefined}
                                                            className={`flex items-center px-3 py-1.5 space-x-1.5 rounded-lg text-xs font-bold border transition-colors ${feedbackMap[originalIdx] === 'incorrect' ? 'bg-neutral-800/80 text-neutral-300 border-neutral-600' : feedbackMap[originalIdx] === 'correct' ? 'opacity-30 cursor-not-allowed bg-black text-neutral-600 border-neutral-800' : 'bg-[#0A0A0A] text-neutral-400 border-neutral-800 hover:text-neutral-300 hover:border-neutral-600'}`}
                                                        >
