...
import { useViz } from '@/hooks/use-viz';
...
export default function Visualizer({ sessionId }: VizProps): React.JSX.Element | null {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-neutral-400">
        ...
        <p>Calculating global distribution matrices...</p>
      </div>
    );
  }
  ...
  const maxRaw = hasNumeric
    ? Math.max(...data.global_raw_hist.map((d: { count: number }) => d.count))
    : 0;
  const maxClean =
    data.global_clean_hist && hasNumeric
      ? Math.max(...data.global_clean_hist.map((d: { count: number }) => d.count))
      : maxRaw;
  const absoluteMax = Math.max(maxRaw, maxClean);
  const healthScore =
    data.health_score ?? (data.global_clean_hist && data.global_clean_hist.length > 0 ? 98 : 45);

  const activeCatData = data?.categorical_data?.find(
    (c: { column: string }) => c.column === selectedCatCol
  );
  const maxCatCount = activeCatData?.top_values
    ? Math.max(...activeCatData.top_values.map((v: { count: number }) => v.count))
    : 0;
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-2xl flex flex-col justify-center">
              <div className="flex items-center text-neutral-400 mb-2">
                <Activity className="w-5 h-5 mr-2 text-blue-500" /> Numeric Features
              </div>
              <p className="text-3xl font-black text-white">{data.columns.length}</p>
            </div>
            <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-2xl flex flex-col justify-center">
              <div className="flex items-center text-neutral-400 mb-2">
                <BarChart2 className="w-5 h-5 mr-2 text-purple-500" /> Distribution Spread
              </div>
            ...
            <div className="bg-emerald-500/10 border border-emerald-500/30 p-6 rounded-2xl flex flex-col justify-center relative overflow-hidden">
              ...
              <div className="flex items-center text-emerald-400 mb-2 font-bold">
                <ShieldCheck className="w-5 h-5 mr-2" /> Data Health Score
              </div>
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-8">
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-sm text-neutral-500 mt-1">
                  Comparing the Z-Score normalization before and after AI sanitization.
                </p>
              </div>
              <div className="flex items-center space-x-4 text-sm font-medium">
                <div className="flex items-center">
                  <div className="w-3 h-3 rounded-full bg-red-500/50 mr-2" /> Raw Data
                </div>
                <div className="flex items-center">
                  <div className="w-3 h-3 rounded-full bg-emerald-500 mr-2 shadow-[0_0_10px_rgba(16,185,129,0.5)]" />{' '}
                  Cleaned Data
                </div>
              </div>
            </div>
            ...
            <div className="relative w-full h-[300px] flex items-end justify-between space-x-1">
              {data.global_raw_hist.map((rawBin: { bin: string; count: number }, idx: number) => {
                ...
                const cleanHeight = cleanBin
                  ? Math.max((cleanBin.count / absoluteMax) * 100, 1)
                  : 0;
                return (
                  <div key={idx} className="relative flex-1 flex flex-col items-center group h-full justify-end">
                    <motion.div
                      initial={{ height: 0 }}
                      animate={{ height: `${rawHeight}%` }}
                      transition={{ duration: 0.8, delay: idx * 0.02 }}
                      className="absolute bottom-0 w-full bg-red-500/30 rounded-t-sm"
                    />
                    {cleanBin && (
                      <motion.div
                        initial={{ height: 0 }}
                        animate={{ height: `${cleanHeight}%` }}
                        transition={{ duration: 0.8, delay: 0.5 + idx * 0.02 }}
                        className="absolute bottom-0 w-full bg-emerald-500/80 rounded-t-sm shadow-[0_-2px_10px_rgba(16,185,129,0.2)]"
                      />
                    )}
                    <div className="opacity-0 group-hover:opacity-100 absolute -top-12 bg-black text-white text-xs py-1 px-2 rounded border border-neutral-800 pointer-events-none whitespace-nowrap z-10 transition-opacity">
                      Bin: {rawBin.bin} <br /> Raw: {rawBin.count} | Clean:{' '}
                      {cleanBin ? cleanBin.count : 'N/A'}
                    </div>
                  </div>
                );
              })}
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
              <div>
                ...
                <p className="text-sm text-neutral-500 mt-1">
                  Explore the most common occurrences in your text data.
                </p>
              </div>
                  ...
                  return (
                    <div key={idx} className="flex items-center">
                      <div className="w-1/3 pr-4 text-right">
                        <span
                          className="text-sm font-medium text-neutral-300 truncate block"
                          title={item.label}
                        >
